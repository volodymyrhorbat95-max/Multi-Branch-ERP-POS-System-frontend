...
const CategoryReport: React.FC = () => {
  ...
  const { categoryReport, loading, error } = useSelector((state: RootState) => state.reports);
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700 animate-fade-down duration-fast">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Venta Prom.</th>
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {categoryReport.categories?.map((category: any, idx: number) => (
                    <tr key={category.category_id || 'uncategorized'} className={`hover:bg-gray-50 dark:hover:bg-gray-700 animate-zoom-in ${idx % 3 === 0 ? 'duration-fast' : idx % 3 === 1 ? 'duration-normal' : 'duration-light-slow'}`}>
